...
async function processInstagramEvent(event: any) {
  ...
  // If listening mode is ON, we receive the webhook (record it mentally/log it) 
  // but we DO NOT call Gemini and DO NOT send a reply.
  if (bot.isListening) {
      console.log(`Bot ${bot.botName} is in Listening Mode. Message received but no reply sent.`);
      return;
  }

  // 2. Generate Reply (Only if NOT listening)
