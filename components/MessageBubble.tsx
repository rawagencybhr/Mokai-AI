    ...
    <div className={`flex w-full mb-4 ${isUser ? 'justify-start' : 'justify-end'}`}>
      <div
        className={`flex flex-col max-w-[85%] md:max-w-[70%] rounded-2xl p-4 shadow-sm relative border ${
          isUser
            ? 'bg-cyan-600 text-white border-cyan-500 rounded-tr-none' // Brand Cyan for User
            : 'bg-white text-slate-800 border-slate-200 rounded-tl-none' // White for Agent
        }`}
      >
        {/* Sender Name Label */}
        <span className={`text-[10px] font-bold mb-1 opacity-80 ${isUser ? 'text-cyan-100' : 'text-cyan-600'}`}>
        ...
        {/* Timestamp */}
        <span className={`text-[10px] mt-2 self-end ${isUser ? 'text-cyan-100' : 'text-slate-400'}`}>
